...
<script type="text/javascript">
    ...
    function ckClick() {
        //取得所有name=ck的被选中的复选框
        var checkedlength = $("input[name=ck]:checked").length;//:checked 过滤只有是被选中的复选框
        //取得所有name=ck的复选框
        var len = $("input[name=ck]").length;
        //进行对比,改变全选复选框的状态
        if (len == checkedlength) {
            $("#all").prop("checked", true);
        } else {
            $("#all").prop("checked", false); // id = all
        }
    }
</script>
<body>
...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            商品名称：<input name="pname" id="pname" value="${prodVo.pname}" >&nbsp;&nbsp;&nbsp;
            商品类型：<select name="typeid" id="typeid">
            <option value="-1">请选择</option>
            <c:forEach items="${typeList}" var="pt">
                <option value="${pt.typeId}" <c:if test="${prodVo.typeid==pt.typeId}">selected="selected"</c:if>>
                        ${pt.typeName}
                </option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="lprice" id="lprice" value="${prodVo.lprice}">-<input name="hprice" id="hprice" value="${prodVo.hprice}">
            <%--不分页查询--%>
            ...
            <c:when test="${info.list.size()!=0}">
                ...
                <div id="top">
                    ...
                    <input type="button" class="btn btn-warning" id="btn1"
                               value="新增商品" onclick="add(${info.pageNum})">
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="edit(${p.pId},${info.pageNum})">编辑
                                    </button>
                    ...
                    <div id="bottom">
                        <div>
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    <li <c:if test="${info.hasPreviousPage == false}"> class="disabled"</c:if>>
                                    ...
                                    <c:if test="${info.pages<=5 }">
                                        ...
                                        <c:set var="over" value="${info.pages}"/>
                                    </c:if>
                                    <c:if test="${info.pages>5 }">
                                        <c:set var="start" value="1"/>
...
<%--                                    <c:set var="over" value="${info.pages}"/>--%>
                                    <c:if test="${info.pageNum>3 && info.pageNum + 2 < info.pages}">
                                        <c:set var="start" value="${info.pageNum - 2}"/>
                                        <c:set var="over" value="${info.pageNum + 2}"/>
                                    </c:if>
                                    <c:if test="${info.pageNum > 3 && info.pageNum + 2 >= info.pages && info.pages >= 5}">
                                        <c:set var="start" value="${info.pages - 4}"/>
                                        <c:set var="over" value="${info.pages}"/>
                                    </c:if>

                                    <c:forEach var="i" begin="${start}" end="${over}" step="1">
                                        <c:if test="${info.pageNum == i}">
                                            <li>
                                                <a class="active"  href="javascript:ajaxsplit(${i}) " style="background-color: hotpink">${i}</a></li>
                                        </c:if>
                                        <c:if test="${info.pageNum != i}">
                                            <li><a href="javascript:ajaxsplit(${i})">${i}</a></li>
                                        </c:if>
                                    </c:forEach>
                                    <li <c:if test="${info.hasNextPage == false}"> class="disabled"</c:if>>
                                        <a href="javascript:ajaxsplit(${info.nextPage})" aria-label="Next">
                                            <span aria-hidden="true">&raquo;</span>
                                        </a>
                                    </li>
                                    <li <c:if test="${info.hasNextPage == false}"> class="disabled"</c:if>>
                                        <a href="javascript:ajaxsplit(${info.pages})" aria-label="Next">
                                            <span aria-hidden="true" >尾页</span>
                                        </a>
                                    </li>

<%--                                    <li >--%>
<%--                                        &lt;%&ndash;  <a href="${pageContext.request.contextPath}/prod/split.action?page=${info.prePage}" aria-label="Previous">&ndash;%&gt;--%>
<%--                                        <a href="javascript:ajaxsplit(${info.prePage})" aria-label="Previous">--%>
<%--                                            <span aria-hidden="true">«</span></a>--%>
<%--                                    </li>--%>

<%--                                    <c:forEach begin="${(info.pages>5&&info.pageNum-2>0)?info.pageNum-2:1}" end="${info.pageNum+2<info.pages?info.pageNum+2:info.pages}" var="i">--%>
<%--                                        &lt;%&ndash;--%>
<%--                                        如果当前页 == 循环页,如果当前页等于循环页就让它背景色改变--%>
<%--                                        &ndash;%&gt;--%>
<%--                                        <c:if test="${info.pageNum==i}">--%>
<%--                                            <li>--%>
<%--                                                    &lt;%&ndash;                                                <a href="${pageContext.request.contextPath}/prod/split.action?page=${i}" style="background-color: grey">${i}</a>&ndash;%&gt;--%>
...
<%--                                        <c:if test="${info.pageNum!=i}">--%>
<%--                                            <li>--%>
<%--                                                    &lt;%&ndash;                                                <a href="${pageContext.request.contextPath}/prod/split.action?page=${i}">${i}</a>&ndash;%&gt;--%>
<%--                                                <a href="javascript:ajaxsplit(${i})">${i}</a>--%>
<%--                                            </li>--%>
<%--                                        </c:if>--%>
<%--                                    </c:forEach>--%>
<%--                                    <li <c:if test="${not info.hasNextPage }">class="disabled" </c:if>>--%>
<%--                                            &lt;%&ndash;  <a href="${pageContext.request.contextPath}/prod/split.action?page=1" aria-label="Next">&ndash;%&gt;--%>
<%--                                        <a href="javascript:ajaxsplit(${info.nextPage})" aria-label="Next">--%>
<%--                                            <span aria-hidden="true">»</span></a>--%>
<%--                                    </li>--%>
                                    <li style=" margin-left:150px;color: #0e90d2;height: 35px; line-height: 35px;">总共&nbsp;&nbsp;&nbsp;<font
                                        ...
                                        跳转到：<input name="switchPage" id="switchPage" style="width: 30px;height: 20px"> 页&nbsp;&nbsp;&nbsp;
                                        <button type="button" class="btn btn-info " style="width: 55px;height: 35px"
                                                onclick="switchPage()">跳转
                                        </button>
                                    </li>
                                </ul>
                            </nav>
...
<script type="text/javascript">
            ...
            if (confirm("您确定删除" + cks.length + "条商品吗？")) {
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                        // var msg = result.extend.DelText.getRecords();
    ...
    function edit(pid,page) {
        ...
        //alert(str);
        // http://localhost/prod/one.action?pid=6&pname=&typeid=2&lprice=undefined&hprice=&page=1
        location.href = "${pageContext.request.contextPath}/prod/edit" + str;
    }
    ...
    function condition() {
        ...
        $.ajax({
            type: "post",
            url: "${pageContext.request.contextPath}/prod/ajaxSplit",// /prod/condition.action
    ...
    function clearCondition(){
        <%--$.ajax({--%>
        <%--    type: "post",--%>
        <%--    url: "${pageContext.request.contextPath}/prod/clearCondition",--%>
        <%--    data: {},--%>
        <%--    success: function () {--%>
        <%--        //刷新数据--%>
        <%--        $("#table").load("${pageContext.request.contextPath}/admin/product.jsp #table");--%>
        <%--    }--%>
        <%--})--%>
        location.href = "${pageContext.request.contextPath}/prod/clearCondition";
